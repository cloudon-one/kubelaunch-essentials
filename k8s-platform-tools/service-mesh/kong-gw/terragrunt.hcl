...
terraform {
  source = "git::https://git@github.com/cloudon-one/k8s-platform-modules.git//k8s-platform-kong-gw?ref=dev"
}
...
inputs = merge(
  local.platform_vars.Platform.Tools[local.tool].inputs,
  {
    ...
    database_subnet_ids  = local.platform_vars.common.database_subnet_ids
  }
)